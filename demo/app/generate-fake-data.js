...
import { getDay, getTime } from 'date-fns'
...
export default function (groupCount = 30, itemCount = 1000, daysInPast = 30) {
  ...
  for (let i = 0; i < itemCount; i++) {
    ...
    const startValue = Math.floor(getTime(startDate) / 10000000) * 10000000
    const endValue = getTime(startDate + faker.random.number({min: 2, max: 20}) * 15 * 60 * 1000)
    ...
    items.push({
      ...
      className: (getDay(startDate) === 6 || getDay(startDate) === 0) ? 'item-weekend' : '',
