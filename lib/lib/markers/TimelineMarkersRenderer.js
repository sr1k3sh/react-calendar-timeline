                    ...
                    return markers.map(function (marker) {
                        switch (marker.type) {
                            case TimelineMarkerType.Today:
                                return (_jsx(TodayMarker, { getLeftOffsetFromDate: getLeftOffsetFromDate, renderer: marker.renderer, interval: marker.interval }, marker.id));
                            case TimelineMarkerType.Custom:
                                return (_jsx(CustomMarker, { renderer: marker.renderer, date: marker.date, getLeftOffsetFromDate: getLeftOffsetFromDate }, marker.id));
                            case TimelineMarkerType.Cursor:
                                return (_jsx(CursorMarker, { renderer: marker.renderer, getLeftOffsetFromDate: getLeftOffsetFromDate }, marker.id));
                            default:
                                return null;
                        }
                    });
